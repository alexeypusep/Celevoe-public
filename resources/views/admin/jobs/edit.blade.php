...
@section('head', 'Редактирование должности')

@section('content')

    <div class="card card-primary">
        <!-- form start -->
        <form action="{{ route('jobs.update', $jobs[0]['id_job'], $jobs[0]['id_job']) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="form-group">
                    <label for="exampleInputName">Организации</label>
                    <select id="exampleInputName" name="id_org" class="form-control">
                        @foreach($organizations as $organization)
                            <option value="{{ $organization->id_org }}" @if($organization->id_org == $jobs[0]['id_org']) selected @endif>{{ $organization->name_org }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="exampleInputDisc">Направления</label>
                    <select id="exampleInputDisc" name="id_direct" class="form-control" onchange="profiladd()">
                        @foreach($directions as $direction)
                            <option value="{{ $direction->id_direct }}" @if($direction->id_direct == $jobs[0]['id_direct']) selected @endif>{{ $direction->name_dir }} ({{ $direction->sphere }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="exampleInputSphere">Сфера деятельности</label>
                    <select id="exampleInputSphere" name="spher" class="form-control" required>
                        @if($jobs[0]['sphere'] == 'Медицинская сфера')
                            <option selected value="Медицинская сфера">Медицинская сфера</option>
                            <option value="Сфера юриспруденции">Сфера юриспруденции</option>
                            <option value="Техническая сфера">Техническая сфера</option>
                        @elseif($jobs[0]['sphere'] == 'Сфера юриспруденции')
                            <option value="Медицинская сфера">Медицинская сфера</option>
                            <option selected value="Сфера юриспруденции">Сфера юриспруденции</option>
                            <option value="Техническая сфера">Техническая сфера</option>
                        @elseif($jobs[0]['sphere'] == 'Техническая сфера')
                            <option value="Медицинская сфера">Медицинская сфера</option>
                            <option value="Сфера юриспруденции">Сфера юриспруденции</option>
                            <option selected value="Техническая сфера">Техническая сфера</option>
                        @endif
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select id="exampleInputProfil" name="profil" class="form-control">
                        <option value="{{ $jobs[0]['profil'] }}">{{ $jobs[0]['profil'] }}</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="exampleInputWww">Название вакансии</label>
                    <input name="position" type="text" class="form-control" id="exampleInputWww" placeholder="Введите название вакансии" value="{{ $jobs[0]['position'] }}" required>
                </div>
                <div class="form-group">
                    <label for="exampleInputPhon">Меры социальной поддержки</label>
                    <textarea name="bonuses" class="form-control" id="exampleInputPhon" placeholder="Введите меры социальной поддержки" required>{{ $jobs[0]['bonuses'] }}</textarea>
                </div>
                <div class="form-group">
                    <label for="exampleInputMinWork">Обязательный срок осуществления трудовой деятельности в организации</label>
                    <input name="min_work" type="text" class="form-control" id="exampleInputMinWork" placeholder="Введите обязательный срок осуществления трудовой деятельности в организации" value="{{ $jobs[0]['min_work'] }}" required>
                </div>
                <div class="form-group">
                    <label for="exampleInputAmJob">Количество вакансий</label>
                    <input name="amount_jobs" type="text" class="form-control" id="exampleInputAmJob" placeholder="Введите количество вакансий" value="{{ $jobs[0]['amount_jobs'] }}" required>
                </div>
            </div>
            <!-- /.card-body -->
