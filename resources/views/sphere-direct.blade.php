...
@section('content')
    ...
    @if(empty($data[0]))
        <h2 class="text-center">В данной ступени образования пока нет направлений</h2>
    @endif
    ...
    @foreach($data as $el)
        <article onclick="location.href='{{ route('direct-one', $el->id_direct) }}'" data-post="191865" class="article article--color-font-dark orgcon mb-4" style="border: 1px solid #e0e0e0" data-post-action-loading-views="yes" data-post-action-loading-bookmarks="yes" data-post-action-loading-reactions="yes" data-post-action-loading-comments="yes">
            <div class="article__main">
                <div class="article__container-text">
                    <div class="article__container-title">
                        <br>
                        <h3 class="article__title article__title--icon">
                            <a class="article__link" style="outline: none; text-decoration: none; color: #2e2e2e;">{{ $el->name_dir }}</a> <span class="badge bg-primary">{{ $amdir[$i] }}</span>
                        </h3>
                    </div>
